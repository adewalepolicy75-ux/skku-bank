...
import { Bolt, Home, Send, User, Coins, CheckCircle, Loader2 } from "lucide-react";
...
export default function SendMoneyPage() {
  ...
  const handleCountryChange = (country) => {
    ...
    setCountryCode(getCountryPrefix(country));
    setToPhone("");
    setReceiverName("");
  };
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-md mx-auto">
            ...
            <form onSubmit={handleSend} className="bg-white rounded-2xl shadow-xl p-8">
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2">
                  <div className="w-20">
                    <input
                      ...
                      value={countryCode}
                      readOnly
                      className="w-full p-3 border border-gray-300 rounded-xl bg-gray-100 text-center"
                    />
                  </div>
                  <input
                    type="tel"
                ...
                {fetching && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <p className="text-xs text-gray-500">Fetching account name...</p>
                  </div>
                )}
                {receiverName && !fetching && (
                  <p className="text-xs text-green-600 mt-1">✓ Sending to: {receiverName}</p>
                )}
                {toPhone && toPhone !== userPhone && !receiverName && !fetching && toPhone.length >= 10 && (
              ...
              <button
                type="submit"
                disabled={loading || fetching || (!receiverName && toPhone.length >= 10)}
                className={`w-full py-3 rounded-xl font-semibold transition flex items-center justify-center gap-2 ${
                  loading || fetching || (!receiverName && toPhone.length >= 10)
